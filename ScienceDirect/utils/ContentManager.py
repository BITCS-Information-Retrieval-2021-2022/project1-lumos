...
class ContentManager():
    ...
    def parse(self, content, paper_url):
        ...
        if soup.find_all(name="span", attrs={"class": "title-text"}):
            title_soup = soup.find_all(name="span",
        ...
        if soup.find(name="div", attrs={"class": "abstract author"}):
            abstract_block = soup.find(name="div",
        ...
        # author
        citation_author_given_name = soup.find_all(
            name="span", attrs={"class": "text given-name"})
        citation_author_surname = soup.find_all(
            name="span", attrs={"class": "text surname"})
        citation_authors = []
        ...
        # pdf_url
        pdfurls = soup.find_all(
            name="a",
            attrs={
                "class":
                "link-button link-button-primary accessbar-primary-link"
            })
        # print(pdfurls)
        pdfurl = ""
        for a in pdfurls:
        ...
        # inCitations 被引数量
        inCitationsstr = soup.find_all(name="h2",
                                       attrs={"class": "section-title u-h4"})
        # cnt = 0
        inCitations = 0
