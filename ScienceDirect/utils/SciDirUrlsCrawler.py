...
class SciDirUrlsCrawler:
    ...
    def getUrlsfromFirstLevel(self, firstlevel: str):
        ...
        try:
            ...
            papers = soup.find_all(name="a",
                                   attrs={"class": "anchor article-content-title u-margin-xs-top u-margin-s-bottom"})
            paperUrls = []
            for paper in papers:
                paperUrl = paper["href"]
                # print(paperUrl)
            ...
            while i < 10:
                ...
                papers_temp = soup_temp.find_all(name="a",
                                                 attrs={
                                                     "class": "anchor article-content-title u-margin-xs-top u-margin-s-bottom"})
    ...
    def getUrlsfromSecondLevel(self, secondlevel: str):
        ...
        try:
            ...
            FirstUrl = soup.find_all(name="a",
                                     attrs={"class": "button-alternative js-listing-link button-alternative-primary"})
            for f_u in FirstUrl:
                url_temp = f_u['href']
    ...
    def getUrlsfromTopLevel(self, toplevel: str):
        ...
        while i:
            print("i:" + str(i))
            temp_url = "https://www.sciencedirect.com/browse/journals-and-books?page=" + str(i) + "&contentType=JL"
            content = self.get_content(temp_url)
            if content is []:
                break
            soup = BeautifulSoup(content, 'lxml')

            SecondUrl = soup.find_all(name="a", attrs={"class": "anchor js-publication-title"})
