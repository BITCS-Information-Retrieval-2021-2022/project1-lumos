...
class ACMUrlsCrawler:
    ...
    def getNextPageAndPapers(self, cururl: str):
        ...
        nextpage = cursoup.find_all(name="a", attrs={"class": "pagination__btn--next"})
        curpage = cursoup.find_all(name="a", attrs={"class": "issue-item__doi dot-separator"})
        nextpageUrl = []
        paperUrls = []
        for cur in curpage:
    ...
    def getNextPageAndProceedings(self, cururl: str):
        ...
        cursoup = BeautifulSoup(content, 'lxml')
        nextpage = cursoup.find_all(name="a", attrs={"class": "pagination__btn--next"})
        curpage = cursoup.find_all(name="div", attrs={"class": "issue-item__content"})
        nextpageUrl = []
        proceedingUrls = []
        for cur in curpage:
    ...
    def getUrlsFromTopLevel(self, toplevel: str):
        ...
        if "journals" in toplevel:
            ...
            # print(soup)
            journalUrls = soup.findAll(name="div", attrs={"class": "browse-item-body"})
            # print(journalUrls)
            ...
            proceedingUrls = soup.find_all(name="a",
                                           attrs={"class": "acm-browse__item-footer", "title": "Proceedings"})
    ...
    def getUrlsfromSecondLevel(self, secondlevel: str):
        ...
        if "journal" in secondlevel:
            ...
            try:
                nextUrl, proceedingUrls = self.getNextPageAndProceedings(secondlevel)
                # print("----" + nextUrl)
    ...
    def getUrlsfromFirstLevel(self, firstlevel: str):
        ...
        if "proceedings" in firstlevel:
            try:
                ...
                soup = BeautifulSoup(content, 'lxml')
                papers = soup.find_all(name="div",
                                       attrs={"class": "issue-item__content-right"})
