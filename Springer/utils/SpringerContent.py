...
class SpringerContent:
    ...
    def getContentfromConferencePaper(self, paperUrl: str):
        ...
        try:
            ...
            content_li = soup.find(name="div",
                                   attrs={"class": "main-body"})

            authors = []
            # pdf_url = content_li.find(name='div', attrs={'c-pdf-download u-clear-both'}).find(name='a')['href']
            pdf_url = self.baseUrl + content_li.find(name='div', attrs={'class': 'note test-pdf-link'}).find(name='a')[
                'href']
            title = str(content_li.find(name='h1', attrs={'class': 'ChapterTitle'}).text).strip()
            co_authors = content_li.find(name='ul', attrs={'class': 'test-contributor-names'}).find_all(name='span',
                                                                                                        attrs={
                                                                                                            'itemprop': 'name'})
            ...
            if content_li.find(name='section', attrs={'class': 'Abstract'}) is not None:
                abstract = str(content_li.find(name='section', attrs={'class': 'Abstract'}).find('p').text).strip()
            # print('Abstract:',abstract)
            pub_time = datetime.datetime.strptime(
                content_li.find(name='div', attrs={'class': 'article-dates'}).find(name='time')['datetime'], '%Y-%m-%d')
            year = pub_time.year
            month = pub_time.month

            doi = content_li.find(name='span', attrs={'id': 'doi-url'}).text.strip()

            # type 会议或期刊
            type = 'conference'

            venue = str(content_li.find(name='a', attrs={'data-track-action': 'Book title'}).text).strip()  # 会议或期刊名
            # print('venue:',venue)
            outcite = content_li.find(name='ol', attrs={'class': 'BibliographyWrapper'})
    ...
    def getContentfromJournalPaper(self, paperUrl: str):
        ...
        try:
            ...
            while content is None:
                ...
                content = self.get_content(paperUrl)
            soup = BeautifulSoup(content, 'lxml')
            content_li = soup.find(name="main",
                                   attrs={"data-track-component": "article body"})

            authors = []

            pdf_url = content_li.find(name='div', attrs={'c-pdf-download u-clear-both'}).find(name='a')['href']
            # pdf_url = soup.find(name='meta', attrs={'name': 'citation_pdf_url'})['content']
            title = str(content_li.find(name='h1', attrs={'class': 'c-article-title'}).text).strip()
            co_authors = content_li.find(name='ul', attrs={'data-test': 'authors-list'}).find_all(name='a', attrs={
            ...
            abstract = ""
            if content_li.find(name='div', attrs={'id': 'Abs1-content'}) is not None:
                abstract = str(content_li.find(name='div', attrs={'id': 'Abs1-content'}).find('p').text).strip()
            pub_time = datetime.datetime.strptime(
                content_li.find(name='ul', attrs={'data-test': 'article-identifier'}).find(name='time')['datetime'],
                '%Y-%m-%d')
            year = pub_time.year
            month = pub_time.month

            doi = content_li.find(name='ul', attrs={'data-test': 'publication-history'}) \
                .find(name='li', attrs={'class': 'c-bibliographic-information__list-item c-bibliographic-information__list-item--doi'}) \
                .find(name='a')['href']
            # type 会议或期刊
            type = 'journal'

            venue = str(content_li.find(name='i', attrs={'data-test': 'journal-title'}).text).strip()  # 会议或期刊名
            outcite = content_li.find(name='ol', attrs={'class': 'c-article-references'})
            cites = outcite.find_all(name='li')
            outCitations = 0  # 引用论文数
            for c in cites:
                outCitations += 1
...
if __name__ == '__main__':
    ...
    urls1 = springerContent.getUnvisitUrls(springerContent.collectionConferenceUrl)
    urls2 = springerContent.getUnvisitUrls(springerContent.collectionJournalUrl)
    ...
    for url_ in pbar:
        ...
        springerContent.updateUrls(url_, springerContent.collectionConferenceUrl)
