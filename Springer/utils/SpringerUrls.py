...
class SpringerUrlsCrawler:
    ...
    def getUrlsfromConference(self, conferencelevel: str):
        ...
        try:
            ...
            # print(soup)
            content_li = soup.find_all(
                name='div', attrs={'class': 'content-type-list__title'})
    ...
    def getUrlsfromJournal(self, journallevel: str):
        ...
        try:
            ...
            content_li = soup.find(name='ol',
                                   attrs={
                                       'id': 'results-list',
                                       'class': 'content-item-list'
                                   }).find_all(name="li", attrs={"class": ""})
            # print('content_li',content_li)
            # 筛选Article的url
            for paper in content_li:
                str_p = str(
                    paper.find(name='p', attrs={
                        'class': 'content-type'
                    }).text).strip()
                # print(str_p)
    ...
    def getUrlsJournalFinal(self, domainlevel: str):
        try:
            ...
            url_final = soup.find(
                name="a",
                attrs={'data-track-action': 'click view all articles'})['href']
    ...
    def getJournalfromTopLevel(self, toplevel: str):
        ...
        try:
            ...
            for domain in tbodies:
                ...
                if result is False:
                    continue
                if not self.hasSecondLevelUrlsInMongoDb(
                        url_final, self.collectionJournalSecondUrl):
                    result, paperUrls = self.getUrlsfromJournal(url_final)
                    if result is False:
                        continue
                    self.saveUrlsToMongoDb(paperUrls,
                                           self.collectionJournalUrl)
                    log("total paper :{length}\n".format(
                        length=len(paperUrls)))
                    self.saveSecondLevelUrlsToMongoDb(
                        url_final, self.collectionJournalSecondUrl)
    ...
    def getConferencefromTopLevel(self, toplevel: str):
        ...
        try:
            ...
            for domain in tbodies:
                ...
                print(self.baseUrl + domain_url)
                if not self.hasSecondLevelUrlsInMongoDb(
                        self.baseUrl + domain_url,
                        self.collectionConferenceSecondUrl):
                    log("=============================================================================\n"
                        )
                    log("Start downloading urls from : " + self.baseUrl + domain_url + "\n")
                    result, paperUrls = self.getUrlsfromConference(
                        self.baseUrl + domain_url)
                    if result is False:
                        continue
                    self.saveUrlsToMongoDb(paperUrls,
                                           self.collectionConferenceUrl)
                    log("total paper :{length}\n".format(
                        length=len(paperUrls)))
                    self.saveSecondLevelUrlsToMongoDb(
                        self.baseUrl + domain_url,
